...
import {
  /* OrbitControls, */ Octahedron,
  Environment,
  Stars /* Replaced Points/PointMaterial with Stars */,
...
export default function CreateDeckPage() {
  ...
  const {
    ...
    query: {
      enabled:
        isConnected && !!address && !!investmentTokenAddress && deckFee > 0n,
    },
  });
  ...
  const needsFeeApproval =
    deckFee > 0n && allowance !== undefined && allowance < deckFee;
  const hasSufficientFeeBalance = balance >= deckFee;

  const { data: approveFeeConfig, error: approveFeeSimError } =
    useSimulateContract({
      address: investmentTokenAddress,
      abi: erc20ABI,
  ...
  const {
    writeContractAsync: approveFeeAsync,
    data: approveFeeTxHash,
    isPending: isApprovingWrite,
  } = useWriteContract();

  const minInvestmentParsed =
    minInvestmentStr && tokenDetails
      ? parseUnits(minInvestmentStr, tokenDecimals)
      : 0n;
  const canPrepareCreate =
    isConnected &&
    !!address &&
    name.trim() !== "" &&
    description.trim() !== "" &&
    minInvestmentParsed >= 0n &&
    (!needsFeeApproval || (allowance !== undefined && allowance >= deckFee)) &&
    !!investmentDeckManagerAddress;

  const { data: createDeckConfig, error: createDeckSimError } =
    useSimulateContract({
      address: investmentDeckManagerAddress,
      abi: investmentDeckManagerABI,
  ...
  const {
    writeContractAsync: createDeckAsync,
    data: createDeckTxHash,
    isPending: isCreatingWrite,
  } = useWriteContract();

  useEffect(() => {
    if (approveFeeSimError)
      console.error("Approve Sim Error:", approveFeeSimError);
  }, [approveFeeSimError]);

  useEffect(() => {
    if (createDeckSimError)
      console.error("Create Sim Error:", createDeckSimError);
  }, [createDeckSimError]);
  ...
  useEffect(() => {
    if (approvalReceipt) {
      ...
      setTimeout(() => {
        setTxState(prev => ({ ...prev, message: null }));
      }, 4000);
    }
  }, [approvalReceipt]);
  ...
  const {
    ...
    isLoading: isWaitingForCreation,
  ...
  useEffect(() => {
    if (creationReceipt) {
      console.log('Deck Creation OK:', creationReceipt.transactionHash);
  ...
  const handleApproveFee = async () => {
    if (!approveFeeConfig?.request) {
      setTxState({
        status: "error",
        message: `Cannot prepare fee approval. ${approveFeeSimError?.shortMessage || ""}`,
      });
      return;
    }
    setTxState({
      status: "loading",
      message: "Check wallet to approve fee...",
    });
    try {
      ...
      setTxState({
        status: "error",
        message: `Fee approval cancelled or failed: ${err.shortMessage || err.message}`,
      });
    }
  };
  ...
  const handleCreateDeck = async () => {
    if (!createDeckConfig?.request) {
      setTxState({
        status: "error",
        message: `Cannot prepare deck creation. ${createDeckSimError?.shortMessage || ""}`,
      });
      return;
    }
    if (deckFee > 0n && !hasSufficientFeeBalance) {
      setTxState({
        status: "error",
        message: `Insufficient ${tokenSymbol} balance for creation fee.`,
      });
      return;
    }
    setTxState({
      status: "loading",
      message: "Check wallet to create deck...",
    });
    try {
      ...
      setTxState({
        status: "error",
        message: `Deck creation cancelled or failed: ${err.shortMessage || err.message}`,
      });
    }
  };
  ...
  const canSubmitApproval =
    approveFeeConfig?.request && !isLoading && hasSufficientFeeBalance;
  const canSubmitCreation =
    createDeckConfig?.request &&
    !isLoading &&
    (deckFee === 0n || (hasSufficientFeeBalance && !needsFeeApproval));
      ...
      <div className="max-w-6xl mx-auto mt-10 px-4">
        <Card className="text-center p-12 max-w-md mx-auto bg-gradient-to-b from-gray-800/80 to-gray-900/80 border border-gray-700">
          ...
          <p className="text-gray-400 text-sm mb-6">
            Please connect your wallet to create an investment deck.
          </p>
          <p className="text-gray-500 text-xs mt-4">
            (Connect Wallet button needed)
          </p>
        </Card>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto mt-10 px-4">
        <Card className="text-center p-12 max-w-md mx-auto bg-gradient-to-b from-gray-800/80 to-gray-900/80 border border-gray-700">
          <p className="text-red-500 text-sm">
            Platform contract address not configured.
          </p>
        </Card>
      </div>
    );
  ...
  if (!tokenDetails.address && deckFee > 0n) {
    return (
      <div className="max-w-6xl mx-auto mt-10 px-4">
        <Card className="text-center p-12 max-w-md mx-auto bg-gradient-to-b from-gray-800/80 to-gray-900/80 border border-gray-700">
          <p className="text-red-500 text-sm">
            Platform token not found. Cannot determine fee requirements.
          </p>
        </Card>
      </div>
    );
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card
          border={true}
          className="!bg-gray-800/80 border-gray-700 p-6 h-full"
        >
          ...
          {!isSuccess ? (
            ...
            >
              ...
              {deckFee > 0n && tokenDetails?.address && (
                <div className="p-4 bg-gray-700/30 rounded-md border border-gray-600/50 text-sm space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span
                      className={`font-medium ${hasSufficientFeeBalance ? "text-gray-100" : "text-red-400"}`}
                    >
                      {isLoadingBalance
                        ? "..."
              ...
              {txState.message && (
                <p
                  className={`text-sm text-center pt-2 font-medium ${
        ...
        <Card
          border={true}
          className="!bg-gradient-to-br from-gray-900 via-indigo-950 to-black border-gray-700/50 p-0 flex flex-col items-center justify-center min-h-[500px] lg:min-h-[600px] overflow-hidden relative group"
        >
          {isSuccess ? (
            ...
            <>
              ...
              <div className="absolute inset-x-0 top-6 sm:top-8 text-center p-4 z-10 pointer-events-none"></div>
              {/* 3D Canvas - Full Card */}
