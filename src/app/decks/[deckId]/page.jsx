...
export default function DeckDetailPage() {
  ...
  const handleFetchAnalysis = async () => {
    ...
    try {
      ...
      // --- Create Prompt (Gemini 2.0 style) ---
      const prompt = `
You are an investment analysis assistant.

Here is a current on-chain snapshot for an investment deck (no historical data, no price charts):
- Deck ID: ${deck.id}
- Name: ${currentDataForPrompt.name}
...
Based only on this snapshot (no external data, no price predictions), return a SINGLE JSON object with exactly these keys:
- "summary": 2–3 sentence plain-language summary of the deck’s current situation (scale, TVL, and profit vs TVL).
- "insights": an array of 2–3 short strings, each a one-sentence observation that clearly states it is based only on the snapshot.
- "riskAssessment": 1–2 sentences describing generic risk based on the description/name and current numbers, explicitly noting it is not historical-performance-based.
- "recommendation": 1–2 cautious sentences that suggest how a user might think about this deck, explicitly telling them to research history and do their own due diligence.
...
- Respond ONLY with the JSON object.`;
      ...
      // --- Call Gemini 2.0 Flash using Axios ---
